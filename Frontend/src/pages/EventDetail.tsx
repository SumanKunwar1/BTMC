  ...
  return (
    <div style={{ background:'#fdf8f3', minHeight:'100vh' }}>
      <style>{`
        ...
        .course-header { display:flex; align-items:center; justify-content:space-between; padding:16px 20px; cursor:pointer; background:#fff; transition:background 0.2s; }
      ...
      {/* Hero */}
      <motion.div style={{ position:'relative', height:'55vh', minHeight:'360px', overflow:'hidden' }} initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.8 }}>
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', justifyContent:'flex-end', padding:'36px', maxWidth:'1200px', margin:'0 auto', left:0, right:0 }}>
          ...
          <h1 style={{ fontFamily:"'Cinzel',serif", fontSize:'clamp(22px,4vw,50px)', fontWeight:900, color:'#1a0808', lineHeight:1.1, marginBottom:'16px', textShadow:'0 1px 8px rgba(253,248,243,0.8)' }}>{event.title}</h1>
                      ...
                      <div key={c.id} className="course-card">
                        ...
                        <AnimatePresence>
                          {isOpen && (
                            ...
                            >
                              <div style={{ padding:'0 20px 20px', borderTop:'1px solid rgba(185,28,28,0.08)', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginTop:'16px' }}>
                                {/* Day 1 */}
                                <div>
          ...
          <motion.div initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.5 }} style={{ position:'sticky', top:'100px' }}>
            <div style={{ background:'#fff', border:'1px solid rgba(185,28,28,0.15)', borderRadius:'6px', overflow:'hidden', boxShadow:'0 8px 30px rgba(185,28,28,0.08)' }}>
              ...
              <div style={{ padding:'18px 20px', display:'flex', flexDirection:'column', gap:'14px' }}>
                ...
                <p style={{ fontFamily:"'Crimson Text',serif", fontSize:'13px', color:'#a07070', textAlign:'center', fontStyle:'italic' }}>We'll confirm your registration</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div
            ...
            style={{ position:'fixed', bottom:'28px', right:'28px', zIndex:100, background:'linear-gradient(135deg,#14532d,#16a34a)', border:'1px solid rgba(22,163,74,0.4)', borderRadius:'4px', padding:'14px 22px', fontFamily:"'Cinzel',serif", fontSize:'11px', letterSpacing:'0.12em', color:'#fff', boxShadow:'0 8px 28px rgba(22,163,74,0.3)' }}
          >

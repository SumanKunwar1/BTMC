// Teachings.tsx — light theme, mobile optimized
...
const Teachings = () => (
  <div style={{ background:'#fdf8f3', minHeight:'100vh' }}>
    <style>{`
      ...
      *, *::before, *::after { box-sizing: border-box; }
      ...
      @media (max-width: 767px) {
        .hero-section { padding: 70px 16px 60px !important; }
        .teachings-section { padding: 48px 16px !important; }
        .info-section { padding: 48px 16px !important; }
        .teachings-grid { grid-template-columns: 1fr; gap: 18px; }
        .info-grid { grid-template-columns: 1fr; }
        .dharma-bg { font-size: clamp(60px, 22vw, 120px) !important; }
      }
    `}</style>

    {/* Hero */}
    <section className="hero-section" style={{ position:'relative', padding:'90px 24px 80px', background:'linear-gradient(180deg,rgba(185,28,28,0.07) 0%,rgba(253,248,243,1) 100%)', textAlign:'center', overflow:'hidden' }}>
      ...
      <div className="dharma-bg" style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', fontFamily:"'Cinzel',serif", fontSize:'clamp(80px,14vw,200px)', fontWeight:900, color:'rgba(185,28,28,0.04)', userSelect:'none', pointerEvents:'none', whiteSpace:'nowrap' }}>DHARMA</div>
      <motion.div initial={{ opacity:0, y:28 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.8 }} style={{ position:'relative', zIndex:2 }}>
        ...
        <h1 style={{ fontFamily:"'Cinzel',serif", fontSize:'clamp(26px,5vw,64px)', fontWeight:900, color:'#1a0808', lineHeight:1.05, marginBottom:'18px' }}>Buddhist Teachings<br/>&amp; Courses</h1>
        <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(15px,2vw,22px)', fontStyle:'italic', fontWeight:300, color:'#a07070' }}>Explore our comprehensive range of courses and begin your spiritual journey</p>
      </motion.div>
    </section>
    ...
    {/* Course Categories */}
    <section className="teachings-section" style={{ maxWidth:'1200px', margin:'0 auto', padding:'80px 24px' }}>
      <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:false }} style={{ marginBottom:'48px' }}>
        ...
        <h2 style={{ fontFamily:"'Cinzel',serif", fontSize:'clamp(20px,2.5vw,34px)', fontWeight:700, color:'#1a0808', marginBottom:'12px' }}>Course Categories</h2>
    ...
    {/* Info blocks */}
    <section className="info-section" style={{ background:'#fef9f4', padding:'80px 24px' }}>
      <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
        <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:false }} style={{ marginBottom:'40px', textAlign:'center' }}>
          <p style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.4em', color:'#b91c1c', textTransform:'uppercase', marginBottom:'10px' }}>How It Works</p>
          <h2 style={{ fontFamily:"'Cinzel',serif", fontSize:'clamp(18px,2.5vw,30px)', fontWeight:700, color:'#1a0808' }}>About Our Classes</h2>
        </motion.div>
